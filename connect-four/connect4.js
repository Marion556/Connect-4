...
function makeHtmlBoard() {
  ...
  //The top row is created and the user can click on top row
  const top = document.createElement("tr");
  top.setAttribute("id", "column-top");
  top.addEventListener("click", handleClick);

  //creates cells 
  //appends cells to the top row
  ...
  // TODO: add comment for this code
  //create the rest of the html board
...
function checkForWin() {
  ...
  for (let y = 0; y < HEIGHT; y++) {
    for (let x = 0; x < WIDTH; x++) {
      // array checks for horizonatal wins
      const horiz = [[y, x], [y, x + 1], [y, x + 2], [y, x + 3]];
      // array checks for vertical wins
      const vert = [[y, x], [y + 1, x], [y + 2, x], [y + 3, x]];
       // array checks for diagonal right wins
      const diagDR = [[y, x], [y + 1, x + 1], [y + 2, x + 2], [y + 3, x + 3]];
      // array checks for diagnonal left wins
      const diagDL = [[y, x], [y + 1, x - 1], [y + 2, x - 2], [y + 3, x - 3]];
        
      // if array returns true then someone won
